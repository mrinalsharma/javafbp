...
public class TestSplitter {
	...
	public static class TestLengthSplitter extends Network {
		private String fixedLength = "{\n" + "  \"splitUsing\": \"Fixed Length\",\n" + "  \"stream\": false,\n"
				+ "   \"length\": 3\n" + "}";
		...
		protected void define() {
			...
			connect("Gen.OUT", "Splitter.IN");
			initialize("1000", component("Gen"), port("COUNT"));
			initialize(fixedLength, component("Splitter"), port("OPTIONS"));
			connect("Splitter.OUT", "Output.IN");

		}
	}

	public static class TestStringSplitter extends Network {
		...
		protected void define() {
			...
			component("Splitter", Splitter.class);
			component("Output", Output.class);
			connect("ReadFile.OUT", "Splitter.IN");
			initialize("src/main/resources/testdata/testdata.txt".replace("/", File.separator), component("ReadFile"), port("SOURCE"));
	...
	public static void main(final String[] argv) throws Exception {
		...
		new TestLengthSplitter().go();
		new TestStringSplitter().go();
